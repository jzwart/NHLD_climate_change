...
#**********************************
# there were a few lakes (~20 out of >44,000) that failed on the condor run or weren't copied over completely. Rerunning those locally with the same script that was run on condor and writing to results folder on Jake's local D: drive 
#*******************************
...
# read in the correct line ID for this run
missing_lakes = readRDS('./R Data/missing_lakes_condor.rds')
missing_lakes$scenario = as.character(missing_lakes$scenario)
missing_lakes$lakes_to_run = as.character(missing_lakes$lakes_to_run)

scenario_lookup <- read.csv('D:/MyPapers/NHLD Climate Change/Results/C_model_output/scenarios.csv',stringsAsFactors = F)

ii = 18 # need to run 1-2, 10-18 - they all fail due to integration issues or uniroot issues; 

# # scenario time period  
period <- strsplit(missing_lakes$scenario[ii], split = '_')[[1]][3]
scenario <- paste(strsplit(missing_lakes$scenario[ii], split = '_')[[1]][1], strsplit(missing_lakes$scenario[ii], split = '_')[[1]][2], sep='_')
scenario_id = scenario_lookup$job[scenario_lookup$scenario==missing_lakes$scenario[ii]]
...
currID = missing_lakes$lakes_to_run[ii]
crun = allruns$crun[allruns$currID==missing_lakes$lakes_to_run[ii]]
...
forceDir<-file.path('F:/Jake/My Papers/NHLD Climate Change/',paste('forcing_files_',scenario_id,sep=''))
# daily lake hydrology flux directory
lakeFluxDir<-file.path('F:/Jake/My Papers/NHLD Climate Change/',paste('input_zipfiles_',scenario_id,sep=''))
# daily lake geomorphology directory 
lakeGeomorphDir<-file.path('F:/Jake/My Papers/NHLD Climate Change/',paste('input_zipfiles_',scenario_id,sep=''))
# daily lake initial conditions directory 
lakeInitGeomorphDir<-file.path('F:/Jake/My Papers/NHLD Climate Change/',paste('input_zipfiles_',scenario_id,sep=''))
...
lakes<-missing_lakes$lakes_to_run[ii]
...
colnames(curForce)<<-c('YYYY','MM','DD','HH','Precip','AirTemp','Longwave','Shortwave','Density','VPD','Pressure','Wind')
  ...
  outDir=file.path('D:/MyPapers/NHLD Climate Change/Results/C_model_output/Condor_Results/',paste('results_',scenario_id,sep=''))
  ...
  saveRDS(out,file = file.path(outDir,paste(crun, "C_model.rds",sep="_")))
